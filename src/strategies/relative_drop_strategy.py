...
from signal_generators.sell_signal_generators.relative_rise_signal import RelativeRiseSignal
...
class RelativeDropStrategy(Strategy):
    def __init__(self, product: str, public_client: cbpro.PublicClient, auth_client: cbpro.AuthenticatedClient, p: float):
        ...
        Arguments:
            ...
            p {float} -- [description]
        """        
        buy_signal_generator = RelativeDropSignal(public_client, product, 24*60*60, p, 0.7)
        sell_signal_generator = RelativeRiseSignal(public_client, product, 0.1)
        risk_allocator = SimplePercentageRiskAllocator(auth_client, product, 0.1)
