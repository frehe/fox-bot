from signal_generators.buy_signal_generators.buy_signal_generator \
    import BuySignalGenerator
...
class RelativeDropSignal(BuySignalGenerator):
    def __init__(
            self, public_client, product, timespan, granularity,
            drop_percentage, max_price_percentage):
        """[summary]
        ...
        self.granularity = granularity  # {60, 300, 900, 3600, 21600, 86400}
    ...
    def getSignal(self) -> BuySignal:
        ...
        while not signal:
            ...
            past_start_time_iso = UnixToISOTimestamp(
                current_epoch - self.timespan)
            past_end_time_iso = UnixToISOTimestamp(
                current_epoch - self.timespan + self.granularity)

            self.past_rate = self._getRate(
                past_start_time_iso, past_end_time_iso)
    ...
    def _relativeDropSignal(self) -> bool:
        ...
        """
        price_dropped = \
            (1.0 - self.drop_percentage) * self.past_rate > self.current_rate
        price_is_low = (
            self.current_rate < self.day_low + self.max_price_percentage
